...
class AplysiaFeedingB:
    ...
    def __init__(self, seaweed_strength = 0.4):
        self.unbroken = 1 # unbroken_adjusted 
        ## Preallocate arrays
    ...
    def Biomechanics_001(self,j):
        ...
        if (self.fixation_type[0,j] ==1):
            ...
            if (not self.unbroken and x_gh <0.3 and x_gh_next>x_gh):
                self.unbroken = 1 

            self.force_on_object[0,j+1]= self.unbroken*self.force_on_object[0,j+1]
    ...
    def GeneratePlots_simplified(self,label,xlimits):
        import math, copy

        t=np.atleast_2d(np.arange(self.StartingTime,self.EndTime,self.TimeStep))
        figure = plt.figure(figsize=(5,1), constrained_layout=True)
        xl=xlimits; # show full time scale
        lineW =2
        ...
        #Grasper Motion plot
        grasper_motion = self.x_g - self.x_h
        plt.plot(t.transpose(),grasper_motion.transpose(),'b',linewidth=lineW)
        ...
        plt.plot(t_gfs.transpose(), grasper_motion_gfs.transpose(),'b', linewidth = lineW * 2)
        ...
        plt.plot(t.transpose(), 1.25 * np.ones_like(t).transpose(), 'k', linewidth = lineW * 3)
        plt.plot(t_delta.transpose(), gm_delta.transpose(),'w', linewidth = lineW * 2.8)     
        plt.plot(t.transpose(),self.force_on_object.transpose(),'k',linewidth=lineW)
        plt.ylabel('Amplitude', color=[0/255, 0/255, 0/255])    
        # i=i+1
